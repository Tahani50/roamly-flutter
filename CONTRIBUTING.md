# Contributing to Roamly Flutter

Thank you for contributing to **Roamly Flutter**.
...
This repository contains the cross-platform **Roamly Flutter** application.

Follow the existing Flutter architecture, state-management approach, dependency-injection setup, routing approach, and folder structure.
...
If code generation is used by the project, run the repository's configured generation command, commonly:

```bash
dart run build_runner build --delete-conflicting-outputs
```

Only run code generation when relevant to your changes.
...
Resolve analyzer warnings introduced by your changes.

### Flutter conventions

- Prefer reusable widgets for repeated UI.
- Keep widgets small and focused.
- Keep business logic outside UI widgets.
- Follow the existing BLoC/Cubit or other state-management pattern.
- Use repository/use-case layers when required by the existing architecture.
- Keep dependency injection consistent with the project.
- Handle loading, success, empty, and error states.
- Do not hardcode reusable colors, spacing, strings, or assets when project constants already exist.
- Keep navigation consistent with the existing router.

### New feature structure

Follow the existing structure. For example:
...
Do not create a new architectural pattern for one feature.

### Flutter branch examples

```text
feature/trip-list
feature/create-trip
fix/trip-card-overflow
refactor/trip-cubit
chore/update-flutter-packages
test/trip-repository
```

### Flutter commit examples

```text
feat: add trip list screen
feat: add create trip cubit
fix: prevent trip card overflow
refactor: simplify trip repository
chore: update flutter dependencies
test: add trip cubit tests
```
